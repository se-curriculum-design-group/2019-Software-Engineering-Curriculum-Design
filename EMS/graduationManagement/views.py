...
def student_submit_project(request):
    ...
    if request.method == "POST":
        ...
        print('document1')
        print()
        if ProjectDocument.objects.filter(schoic=stu, dtype=dtype).count() == 0:
            print('document2')
...
def teacher_submit_project(request):
    ...
    if request.method == 'POST':
        ...
        if 'change' not in request.get_full_path():
            ...
            id = request.GET.get('change_id')
            GraduationProject.objects.filter(id=id).update(pname=name)
            ...
            GraduationProject.objects.filter(tno=tno, id=id).update(pdescription=descripe)
...
def teacher_view_projectfiles(request):
    ...
    if request.method == 'POST':
        ...
        ProjectScore.objects.filter(project=id).update(lundian=int(lundian), fangan=int(fangan), dabian=int(dabian),
                                                       comments=comments)
...
def admin_view_project_detail(request):
    ...
    info = GraduationProject.objects.filter(id=id)
    contenxt = {
        'info': info,
    }
